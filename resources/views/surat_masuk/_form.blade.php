...
<div class="row">
    ...
    <div class="col-md-6 mb-3">
        ...
        <input type="date" name="tanggal_diterima" class="form-control @error('tanggal_diterima') is-invalid @enderror"
            value="{{ old('tanggal_diterima', isset($suratMasuk) ? $suratMasuk->tanggal_diterima->format('Y-m-d') : '') }}"
            required>
...
<div class="row">
    <div class="col-md-6 mb-3">
        ...
        <select name="sifat_surat" class="form-select @error('sifat_surat') is-invalid @enderror" required>
            ...
            @foreach(['Biasa', 'Penting', 'Rahasia'] as $sifat)
                <option value="{{ $sifat }}" {{ old('sifat_surat', $suratMasuk->sifat_surat ?? '') == $sifat ? 'selected' : '' }}>{{ $sifat }}</option>
            @endforeach
        </select>
    ...
    <div class="col-md-6 mb-3">
        ...
        <input type="text" name="klasifikasi" class="form-control @error('klasifikasi') is-invalid @enderror"
            value="{{ old('klasifikasi', $suratMasuk->klasifikasi ?? '') }}" required>
...
<div class="mb-3">
    ...
    @if(isset($suratMasuk) && $suratMasuk->file_surat)
        <small class="text-muted">File saat ini: <a
                href="{{ route('surat-masuk.download', $suratMasuk->id) }}">{{ basename($suratMasuk->file_surat) }}</a></small>
    @endif
...
<div class="mb-3">
    ...
    <textarea name="keterangan" class="form-control"
        rows="2">{{ old('keterangan', $suratMasuk->keterangan ?? '') }}</textarea>
</div>